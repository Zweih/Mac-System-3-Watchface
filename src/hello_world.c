...
static void init(void) {
  ...
  // Set the font and text alignment
	GFont Chicago = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_Font_Chicago_36));
	
	s_text_layer = text_layer_create(GRect(3, 60, 121, 41));
	text_layer_set_font(s_text_layer, Chicago);
	text_layer_set_text_alignment(s_text_layer, GTextAlignmentCenter);
	
	GFont goth = fonts_get_system_font(FONT_KEY_GOTHIC_14);
	date_layer = text_layer_create(GRect(61, 43, 68, 16));
